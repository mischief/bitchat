...
size_t
bc_emoji_lookup(const char *name, size_t namelen, char out[5])
{
	size_t lo = 0, hi = bc_emoji_table_len;
	while (lo < hi) {
		size_t mid = lo + (hi - lo) / 2;
		const char *cand = bc_emoji_entry_name(mid);
		...
		if (cmp == 0) {
			size_t n = put_utf8(
			    unpack_codepoint(bc_emoji_table[mid].cp_packed),
			    out);
			out[n] = '\0';
			return n;
		} else if (cmp < 0)
