...
-- bc_emoji_table char* version this replaced -- 1410 entries * 2 pointers
-- meant 2820 R_X86_64_RELATIVE fixups at every process start):
--   - bc_emoji_names: one NUL-separated blob of every shortcode name.
--   - bc_emoji_table: { uint16_t name_off; uint16_t cp_packed } per entry,
...
out:write("const unsigned char bc_emoji_names[] = {\n")
for i = 1, #blob, 16 do
	local bytes = {}
...
out:write("const struct bc_emoji_entry bc_emoji_table[] = {\n")
for _, e in ipairs(entries) do
...
out:write(string.format(
    "const size_t bc_emoji_table_len = %d;\n", #entries))
out:close()
